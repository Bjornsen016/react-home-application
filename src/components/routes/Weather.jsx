...
import {useLocation} from "react-router-dom";
  ...
  const Weather = () => {
    ...
    const fetchUserPosition = new URLSearchParams(location.search).get('location')

    //Array of pre-chosen locations to fetch data for 
    const locations = [{name:"", data: data0, cityName: userPosition }, {name:"Göteborg", data: data1, cityName: data1City}, {name:"Stockholm", data: data2, cityName: data2City}, {name:"Malmö", data: data3, cityName: data3City}]
    ...
    const urlSearchedLocation = `https://api.openweathermap.org/data/2.5/forecast?q=${searchedLocation}&appid=${apiKey}&units=metric`
      ...
      useEffect(() => {
      setUserPosition(fetchUserPosition);
      fetchWeatherData();
    ...
    return (
      ...
      {/* UserSearchData, only shown if showSearchedLocation=true */}
                 ...
                 <TableRow>
                  <TableCell component="th" scope="row" >{searchedLocationCity}</TableCell>
                  {
                    searchedLocationData?.map(({main, weather, dt_txt, city}) => (
